...
#             * |mu|^{-(p+1)/2}
#
#   Proper:   (alpha - (p-1)/2) ~ Gamma(beta, rate = 
#             beta*eta)
...
# density.
#
# REQUIREMENT: none beyond an internet connection the
# first time you run this. The wishartinference package
...
#   - Proper:   provide mu0, beta >= 0, kappa >= 1
# =======================================================
...
# no prior parameters are provided. Under the current 
# proper-prior parameterization -- (alpha-(p-1)/2) ~ 
# Gamma(beta, rate = beta*eta) -- setting beta=0, eta=1, 
# kappa=0 is actually mathematically consistent with (not 
# just a coincidental stand-in for) the improper prior: 
# beta=0 collapses (alpha-(p-1)/2)^(beta-1) to 
# (alpha-(p-1)/2)^{-1} and exp(-beta*eta*(alpha-(p-1)/2)) 
# to 1 for any eta (since beta multiplies eta in the rate), 
# and kappa=0 removes mu0's influence entirely -- exactly 
# recovering the improper prior above. These are also the 
# function defaults, which is why omitting mu0, beta, eta, 
# kappa triggers the same improper path internally.
# -------------------------------------------------------
...
# PROPER PRIOR
#
# Specify beta >= 0, kappa >= 1, and mu0.
# The prior is (alpha - (p-1)/2) ~ Gamma(beta, rate =
# beta*eta), so for beta >= 1 the prior mode of alpha is
# (p-1)/2 + (beta-1)/(beta*eta) -- note the shift by the
# domain boundary (p-1)/2, unlike the old truncated-Gamma
# prior where the mode was (beta-1)/(beta*eta) directly.
# Here we place the prior mode at 3, while the true alpha
# is 2, to demonstrate prior-likelihood tension.
#
...
eta   <- (beta-1)/(beta*(3-(p-1)/2)) # prior mode = (p-1)/2 + (beta-1)/(beta*eta) = 3
kappa <- 1                 # prior strength on mu
mu0   <- diag(10, p)        # prior center for mu (= true mu)

res_pro <- wishart_inference(X, mu0 = mu0, beta = beta,
                             eta = eta, kappa = kappa, nsamp = 50000)

cat("--- Proper Prior ---\n")
cat("prior mode:           ", (p - 1) / 2 + (beta - 1) / (beta * eta), "\n")
cat("true a:               ", a, "\n")
...
prior_pro <- sapply(alpha_grid_pro, function(av) {
  ...
  dgamma(av - (p - 1) / 2, beta, rate = beta * eta)
})
...
# =======================================================
# The package backend validates its inputs and its own
# internal convergence at every layer. This section
# deliberately triggers each category of error so you can
# see the messages you'll get back, and confirm error
# handling behaves consistently before you rely on it.
# Every call below is wrapped in tryCatch() so this script
# keeps running; in normal use you'd just let the error
# propagate (or check for wishart_inference()'s $error
# field -- see 4H below).
#
...
# beta must be >= 0, eta must be >= 0, kappa must be >= 1,
# and all three must be finite. (The prior is now the shifted
# Gamma (alpha-(p-1)/2) ~ Gamma(beta, rate=beta*eta), which is
# proper for any beta >= 0, eta >= 0 -- there is no longer a
# beta > 1 requirement, nor a prior-mode-vs-boundary condition
# to check.)
demo_error("beta < 0 rejected", {
  wishart_inference(X, mu0 = diag(p), beta = -1, eta = 1, kappa = 1, nsamp = 100)
})
demo_error("eta < 0 rejected", {
  wishart_inference(X, mu0 = diag(p), beta = 5, eta = -1, kappa = 1, nsamp = 100)
})
...
# =======================================================
# SECTION 5: Gamma Case (p = 1)
# =======================================================
# The "gamma" model is just Wishart_p(2*alpha, Sigma) at
# p = 1 -- each observation is a positive scalar rather than
# a matrix, so gamma_inference() takes a plain numeric vector
# instead of a p x p x n array. Improper/proper prior are
# auto-detected exactly as in wishart_inference(). There's no
# unifying dispatch between the two -- call gamma_inference()
# directly for scalar data, wishart_inference() directly for
# matrix data.
# =======================================================
...
# improper prior (default)
res_g_imp <- gamma_inference(x_g, nsamp = 20000)
cat("--- Gamma (p=1), Improper Prior ---\n")
cat("true a:               ", a_g, "\n")
cat("posterior mode:       ", res_g_imp$results$ahat, "\n")
cat("95% Interval:         ", quantile(res_g_imp$results$alpha_samples, c(0.025, 0.975)), "\n")
cat("theoretical acpt rate:", res_g_imp$results$theoretical_acpt_rate, "\n")
cat("empirical acpt rate:  ", res_g_imp$results$empirical_acpt_rate, "\n")

# proper prior
res_g_pro <- gamma_inference(x_g, mu0 = 1, beta = 5, eta = 0.5, kappa = 2, nsamp = 20000)
cat("\n--- Gamma (p=1), Proper Prior ---\n")
cat("true a:               ", a_g, "\n")
cat("posterior mode:       ", res_g_pro$results$ahat, "\n")
